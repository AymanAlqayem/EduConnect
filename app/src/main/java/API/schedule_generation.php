            ...
            for ($i = 0; $i < $periods; $i++) {
                ...
                foreach ($timeslots as $slot) {
                    ...
                    if (
                        ...
                        $stmt = $pdo->prepare("INSERT INTO schedules (section_id, subject_id, timeslot_id, teacher_id, class_id, created_by) VALUES (?, ?, ?, ?, ?, ?)");
                ...
                if (!$slotAssigned) {
                    throw new Exception("Unable to assign all periods for subject ID $subject_id in section ID $section_id.");
                }
            }
        }
    }

    echo json_encode(['status' => 'success', 'message' => 'Schedule generated successfully.']);
} catch (Exception $e) {
